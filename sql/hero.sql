...
SELECT * FROM superhero;
 ...
 DELIMITER $$
CREATE PROCEDURE spu_heroe_listar(IN _publisher_id INT)
BEGIN
	SELECT 
		SUP.id,
        SUP.superhero_name,
...
CALL spu_heroe_listar(4);

DROP PROCEDURE spu_heroe_listar;
...
BEGIN
    SELECT
        a.alignment, COUNT(s.id) AS total
    FROM superhero.alignment a
    LEFT JOIN superhero.superhero s ON a.id = s.alignment_id
...
DROP PROCEDURE spu_resumen_alignment;
...
BEGIN
    SELECT superhero.id, superhero.superhero_name, superhero.full_name, gender.gender_name, race.race_name
    FROM superhero.superhero
    LEFT JOIN superhero.gender ON superhero.gender_id = gender.id
    LEFT JOIN superhero.race ON superhero.race_id = race.id
    WHERE superhero.publisher_id = publisherId;
END //
DELIMITER ;


DELIMITER //
CREATE PROCEDURE GetHeroesByAlignment(
    IN publisherIdParam INT
)
BEGIN
    DECLARE publisherId INT;
    
    SELECT id INTO publisherId
    FROM superhero.publisher
    WHERE id = publisherIdParam;

    IF publisherId IS NOT NULL THEN
        SELECT a.alignment, CAST(COUNT(*) AS UNSIGNED) AS numHeroes
        FROM superhero.superhero h
        INNER JOIN superhero.alignment a ON h.alignment_id = a.id
        WHERE h.publisher_id = publisherId
        GROUP BY a.alignment;
    ELSE
        SELECT 'Publisher no encontrado' AS result;
    END IF;
END //
DELIMITER ;
...
CALL GetHeroesByAlignment(4);
DROP PROCEDURE GetHeroesByAlignment;

DELIMITER $$
CREATE PROCEDURE spu_listar_bandosP(IN _id INT)
BEGIN
	SELECT 
...
DROP PROCEDURE spu_listar_bandos_por_publishers;
